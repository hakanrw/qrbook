...
{#if data.type === "text"}
  <textarea class="textarea textarea-bordered flex-grow" placeholder="text" on:change={(e) => onUpdate(e.target.value)} use:init>{data.value}</textarea>
{:else if data.type === "image"}
  <div class="w-full">
    <input accept="image/*" type="file" class="file-input file-input-bordered w-full " on:change={(e) => onUpdate(URL.createObjectURL(e.target.files[0]))} use:init />
    {#if data.value}
...
{:else if data.type === "kvpair"}
  <div class="flex-grow">
    {#each data.value as entry, i}
      <div class="flex my-2">
        <input type="text" autocomplete="new-password" placeholder="key" class="grid w-24 sm:w-48 input input-bordered" use:init on:change={(e) => onUpdate( data.value.map((kv, index) => index === i ? [e.target.value, entry[1]] : kv) ) } value={entry[0]}  />
